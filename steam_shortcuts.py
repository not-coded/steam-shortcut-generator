...
def main():
    ...
    start_menu = (
        input("\nAdd shortcuts to a Start Menu folder? y/[N] ")
        .lower()
        .strip()
        == "y"
    )
    ...
    try:
        ...
        print(
            "\n\nUnable to add shortcuts to the start menu, try running this tool as sudo."
        )
...
def get_steam_path():
    ...
    # ~/.local/share/Steam
    steam_path = os.path.join(os.path.expanduser("~"),".local", "share", "Steam")

    # Ask the user if the registry was unhelpful
    if not steam_path or not os.path.isdir(steam_path):
        steam_path = input(
            "Failed to find Steam installation path, please provide the path e.g. ~/.local/share/Steam, ~/.steam/steam, etc.\n"
        )

    # Actually be able to use "~" for the path
    if (steam_path.startswith("~/")):
        steam_path = os.path.join(os.path.expanduser("~"), steam_path[2:])
...
def check_for_icons(games):
    ...
    for an icon.jpg in the game's installation directory
    """

    for appid, game in games.items():
        try:
            icon_path = pathlib.Path(game["location"] / f"{game['icon_hash']}.jpg")
...
def create_shortcuts(games, create_with_missing, start_menu=False):
    ...
    for appid, game in games.items():
        ...
        name = re.sub(r'[\\/*?:"<>|]', "", game["name"])
        filename = name + ".desktop"

        # Skip game if missing the icon and the user asked to
        # not create shortcuts with missing icons
        if not game["icon"]:
            if not create_with_missing:
                continue
            game["icon"] = "steam" # Default to the steam icon when no icon found (Steam does this too)
            
        # Write the shortcut file
        with open(folder / filename, "w+", encoding="utf-8") as shortcut:
            shortcut.write("[Desktop Entry]\n")
            shortcut.write(f"Name={name}\n")
            shortcut.write("Comment=Play this game on Steam (generated by Steam Shortcut Generator)\n")
            shortcut.write(f"Exec=steam steam://rungameid/{appid}\n")
            shortcut.write(f"Icon={game['icon']}\n")
            shortcut.write("Terminal=false\n")
            shortcut.write("Type=Application\n")
            shortcut.write("Categories=Game;\n")
        count += 1

        os.chmod(folder / filename, os.stat(folder / filename).st_mode | S_IEXEC)
